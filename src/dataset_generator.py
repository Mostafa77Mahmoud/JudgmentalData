...
"""
    ...
    def _batch_verify_examples(self, examples: List[Dict],
    ...
    def run_smoke_test(self, language: str, target_count: int = 15) -> Dict:
        ...
        # Save summary report
        summary_file = self._save_smoke_test_summary(language, stats,
                                                     valid_examples,
                                                     failed_raw_paths)
        ...
        if stats["fabrication_rate"] > MAX_FABRICATION_RATE:
            self.logger.warning(
                f"High fabrication rate detected: {stats['fabrication_rate']:.2%} > {MAX_FABRICATION_RATE:.2%}"
            )
            return {
                "success":
                False,
                ...
                failed_raw_paths[:3],
                "error":
                f"Fabrication rate too high: {stats['fabrication_rate']:.2%} > {MAX_FABRICATION_RATE:.2%}"
            }
    ...
    def generate_full_dataset(self,
        ...
        while len(all_examples) < target and processed_chunks < len(chunks):
            ...
            if batch_examples:
                # Verify examples
                verified_batch = self._batch_verify_examples(
